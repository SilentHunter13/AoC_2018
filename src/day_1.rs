...
pub fn star_2() -> i32 {
    ...
    if step > 0 {
        ...
        let mut smallest_divisor = i32::max_value();
        let mut smallest_index = usize::max_value();
        let mut value = 0;
        for x in 0..(sum_list.len() - 1) {
            for y in (x + 1)..sum_list.len() {
                let diff = (sum_list[x] - sum_list[y]).abs(); //möglicherweise nicht nötig
                let divisor = diff / step;
                if (diff % step) == 0 {
                    //an der Stelle des kleineren Index wird der größere Wert erreicht
                    //größeren Wert Index des kleicheren Wertes bestimmen
                    let tuple = if sum_list[x] > sum_list[y] {
                        (sum_list[x], y)
                    } else {
                        (sum_list[y], x)
                    };

                    match divisor.cmp(&smallest_divisor) {
                        Ordering::Less => {
                            smallest_divisor = divisor;
                            smallest_index = usize::max_value();
                            value = tuple.0;
                        }
                        Ordering::Equal => {
                            if tuple.1 < smallest_index {
                                smallest_index = tuple.1;
                                value = tuple.0;
                            }
                        }
                        Ordering::Greater => {}
